...
public class DriveStraight extends Command {
    ...
    protected void execute() {
    	...
    	RobotMap.driveLeftTop.set(ControlMode.PercentOutput, 0.6 * (-vel - head));
		RobotMap.driveRightTop.set(ControlMode.PercentOutput, 0.6 * (vel - head));
    ...
    protected boolean isFinished() {
		if(!scoreHatch) 
			...
			return isTimedOut() && (Constants.isWithinThreshold(RobotMap.wristControl.getSelectedSensorPosition(0), Elevator.elevatorState.getClawPosition() + 1300, Elevator.elevatorState.getClawPosition() + 1700));
	}
